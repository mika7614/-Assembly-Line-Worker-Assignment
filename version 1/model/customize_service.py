...
# class OperationAssignment:
    ...
    def algorithm(self, data):
        ...
        station_code_to_line_number = {i:j for j, i in station_id_dict.items()}
        ######################################################################################
        ...
        for i in range(num_stations):
            ...
            for j in range(num_machines):
                t.append(model.NewIntVar(0, mmps, f'y[{i},{j}]'))
            y.append(t)
        ...
        for i in available_stations:
            ...
            for j in range(num_machines):
                ...
                if j in curr_machine_nums:
                    model.AddAbsEquality(curr_machine_nums[j], y[i][j])
                    continue
                if not config["is_movable"]:
                    model.AddAbsEquality(0, y[i][j])
                    continue
                if config["is_mono"]:
                    temp_var = model.NewBoolVar(f"(mono_y[{i},{j}])")
                    model.Add(y[i][j] >= 1).OnlyEnforceIf(temp_var)
                    model.Add(y[i][j] < 1).OnlyEnforceIf(temp_var.Not())
                    model.Add(sum(y[i]) == 1).OnlyEnforceIf(temp_var)

        ## 建立有关工位设备数量的约束
        ### Each station has mmps machines at most.
        for i in available_stations:
            model.Add(sum(y[i]) <= mmps)
        ###########################################################################################################
        ...
        costs = 100000 * np.ones((num_workers, num_tasks))

        for i in range(num_workers):
            available_tasks = set(worker_efficiency_dict[i].keys())
            for j in range(num_tasks):
                operation = oper_id_dict[j]
                if operation not in available_tasks:
                    continue
                costs[i,j] = oper_time_dict[j] / worker_efficiency_dict[i][operation]
        ...
        for i in range(num_workers):
            for j in range(num_tasks):
                if costs[i][j] != 100000:
                    continue
                model.AddAbsEquality(0, x[i][j])

        # Constraints
        # Each worker is assigned to at least one task.
        for i in range(num_workers):
            model.AddAtLeastOne(x[i][j] for j in range(num_tasks))

        # Each task is assigned to at least one worker.
        for j in range(num_tasks):
            model.AddAtLeastOne(x[i][j] for i in range(num_workers))

        # Objective
        objective_terms = [0] * num_workers
        for i in range(num_workers):
            for j in range(num_tasks):
                objective_terms[i] += costs[i][j] * x[i][j]
        ###########################################################################################################
        ...
        for i in range(num_machines):
            ...
            for j in range(num_stations):
                ...
                for k in range(num_tasks):
                    t.append(model.NewBoolVar(f'mst[{i},{j},{k}]'))
                r.append(t)
            mst.append(r)

        aux_ms = [[None for j in range(num_stations)] for i in range(num_machines)]
        for i in range(num_machines):
            for j in available_stations:
                ...
                for g_id, oper_group in enumerate(machine_to_oper[i]):
                    sub_aux_ms = [mst[i][j][oper_id] for oper_id in oper_group]
                    temp_aux_ms = model.NewBoolVar(f"(temp_aux_ms[{i},{j}])")
                    model.Add(sum(sub_aux_ms) >= 1).OnlyEnforceIf(temp_aux_ms)
                    model.Add(sum(sub_aux_ms) < 1).OnlyEnforceIf(temp_aux_ms.Not())
                    curr_aux_ms = model.NewBoolVar(f"(aux_ms_g{g_id}[{i},{j}])")
                    model.Add(curr_aux_ms == 1).OnlyEnforceIf(temp_aux_ms)
                    model.Add(curr_aux_ms == 0).OnlyEnforceIf(temp_aux_ms.Not())
                    temp.append(curr_aux_ms)
                aux_ms_in_curr_station = model.NewIntVar(0, mmps, f'aux_ms_ics[{i},{j}]')
                model.Add(aux_ms_in_curr_station == sum(temp))
                aux_ms[i][j]= aux_ms_in_curr_station
        ...
        for i in range(num_machines):
            if i not in machine_to_oper:
                continue
            for j in available_stations:
                model.Add(y_ms[i][j] >= aux_ms[i][j])
        ###########################################################################################################

        # auxiliary variable swt
        ###########################################################################################################
        ## station-worker-task
        swt = []
        for i in range(num_stations):
            ...
            for j in range(num_workers):
                t = []
                for k in range(num_tasks):
                    t.append(model.NewBoolVar(f'swt[{i},{j},{k}]'))
                r.append(t)
            swt.append(r)

        ### swt to sw
        sw = [[None for j in range(num_workers)] for i in range(num_stations)]
        for i in available_stations:
            for j in range(num_workers):
                temp_sw = model.NewBoolVar(f'temp_sw[{i},{j}]')
                model.Add(sum(swt[i][j]) >= 1).OnlyEnforceIf(temp_sw)
                model.Add(sum(swt[i][j]) < 1).OnlyEnforceIf(temp_sw.Not())
        ...
        ### stw => mst
        ###############################################################
        st = [[None for j in range(num_tasks)] for i in range(num_stations)]
        for i in available_stations:
            for j in range(num_tasks):
                temp_st = model.NewBoolVar(f"(temp_st[{i},{j}])")
                model.Add(sum(stw[i][j]) >= 1).OnlyEnforceIf(temp_st)
                model.Add(sum(stw[i][j]) < 1).OnlyEnforceIf(temp_st.Not())
        ...
        # Answer
        ###########################################################################################################
        # print(sum(objective_terms))
        sum_time = model.NewIntVar(1, 1000000, "sum_time")
        model.AddAbsEquality(sum(objective_terms), sum_time)
        ...
        model.AddMaxEquality(max_time, objective_terms)
        ...
        ###############################################################
        workers_time = [None] * num_workers
        workers_time_power = [None] * num_workers
        for i in range(num_workers):
            worker_time_id = f"w{i}_time"
            workers_time[i] = model.NewIntVar(1, int(1e6), worker_time_id)
            worker_time_power_id = f"w{i}_time_power"
            workers_time_power[i] = model.NewIntVar(1, int(1e12), worker_time_power_id)
            model.AddAbsEquality(workers_time[i], objective_terms[i])
        ...
        model.AddAbsEquality(sum(workers_time_power), sum_of_time_power)
        ...
        ## score2
        model.AddDivisionEquality(score2, scale*var_time, power_of_mean_time)
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
            ...
            for i in available_stations:
                ...
                for j in range(num_workers):
                    ...
                    for k in range(num_tasks):
                        temp = {}
                        if solver.BooleanValue(swt[i][j][k]):
                            line_number = i+1
                            station_code = station_id_dict[line_number]
                            ...
                            temp["operation_number"] = count
                            operation_list.append(temp)
                            count += 1
                    if operation_list:
                        one_station_result["operation_list"] = operation_list
                if one_station_result:
...
#     a = OperationAssignment('a', 'b')
#     f = open("训练集/instance-1.txt", "r")
#     data = {0: {0: f}}
#     result = a._inference(data)
#     f.close()
#     print(result)